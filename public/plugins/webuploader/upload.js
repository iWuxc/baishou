		...
		upload : function(opts){
			...
			if(type == 'Images'){
				...
				this.opts.accept = {
					...
					mimeTypes : 'image/gif,image/jpg,image/jpeg,image/png,image/bmp,application/pdf'
		...
		createFile : function(data){
			...
			if(data && data.list && data.list.length > 0){
				...
				for(; i < data.list.length; i++){
					...
					if(!/png|jpg|jpeg|gif|bmp|pdf/.test(ext)) file = '<div class="img file-global file-' + ext + '" title="' + t.name + '"><span class="icon"></span></div>';
