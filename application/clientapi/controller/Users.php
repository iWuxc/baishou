...
class Users extends Base {
    ...
    public function updateHeadPic(){
        ...
        if($file){
            ...
            $HeadPic = '/public/upload/head/'.$info->getSaveName();
            $QnuUploadPic = $this -> uploadFileQnu($HeadPic);
            if(!$QnuUploadPic){
                $this -> _toJson("0", '上传失败', (object)array());
            }
            if($type == 1){
                $where['user_id'] = $userid;
                $result = M('users')->where($where)->update(['head_img'=>$QnuUploadPic,'update_time'=>time()]);
            }else{
                $where['clerk_id'] = $userid;
                $result = M('user_store_clerk')->where($where)->update(['head_img'=>$QnuUploadPic,'update_time'=>time()]);
            }
            if($result){
                $this -> _toJson("1", '上传成功', array('head_pic'=>$QnuUploadPic));
            }else{
                $this -> _toJson("0", '上传失败', (object)array());
            }
        }else{
     ...
     * @return bool|string
     */
    public function uploadFileQnu($temp_url){
        $url = qiniuuploadv($this->rootPath.$temp_url);
        if($url){
            @unlink($this->rootPath.$temp_url);
            return $url;
        }
        return false;
    }
