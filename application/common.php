 ...
 */ 
function send_email($to,$subject='',$content='',$type='coder'){
     //发送邮件 
    if('acg' == $type){
        $smtp_config = C('EMAIL_ACG');
    }elseif('baishou' == $type){
        $smtp_config = C('EMAIL_BAISHOU');
    }else{
        $smtp_config = C('EMAIL_CODER');
    } 
 ...
 * 发送短信逻辑
 * @param $scene 短信类型编号
 * @param $sender 发送的手机号码
 * @param $params 发送的内容参数
 * @param int $unique_id
 * @return array
 */
//write by iWuxc 引用于阿里大鱼
function sendSms($scene, $sender, $params, $unique_id=0)
{
    //用到那个参数就在$param中加入就可以了
    $code = !empty($params['code']) ? $params['code'] : false;
    $name = !empty($params['name']) ? $params['name'] : false;
    $passwd = !empty($params['password']) ? $params['password'] : false;
    $mobile = !empty($params['mobile']) ? $params['mobile'] : false;
    $date = !empty($params['date']) ? $params['date'] : false;
    $apply_mount = !empty($params['apply_mount']) ? $params['apply_mount'] : false;
    $new_value = !empty($params['new_value']) ? $params['new_value'] : false;
    if(empty($unique_id)){
        $session_id = session_id();
    }else{
        $session_id = $unique_id;
    }
    $smsParams = array(//根据业务的发送场景自定义
        1 => "{\"name\":\"$name\",\"mobile\":\"$mobile\",\"passwd\":\"$passwd\"}",//boss注册成功
        2 => "{\"code\":\"$code\"}",//2. 用户注册 (验证码类型短信只能有一个变量)
        3 => "{\"name\":\"$name\",\"date\":\"$date\",\"apply_mount\":\"$apply_mount\"}",//提款成功-信贷
        4 => "{\"name\":\"$name\",\"date\":\"$date\"}",//提款成功-客户
        5 => "{\"name\":\"$name\",\"date\":\"$date\",\"new_value\":\"$new_value\"}",//出库成功-客户
        6 => "{\"name\":\"$name\",\"date\":\"$date\",\"new_value\":\"$new_value\"}",//出库成功-信贷
        7 => "{\"name\":\"$name\",\"date\":\"$date\"}",//出库成功-客户
        8 => "{\"name\":\"$name\",\"date\":\"$date\"}",//出库成功-信贷
        9 => "{\"name\":\"$name\"}",//授信成功-信贷
        10 => "{\"name\":\"$name\"}",//授信成功-风控
        11 => "{\"name\":\"$name\"}",//授信成功-信贷
        12 => "{\"name\":\"$name\"}",//授信成功-风控
        13 => "{\"name\":\"$name\"}",//出库申请-管理端
        14 => "{\"name\":\"$name\"}",//授信申请-管理端
    );
 ...
 * @return bool
 */
function PushMessage($type, $cid, $title, $params, $sub_type = 'client')
{
    //用到那个参数就在$param中加入就可以了
    $name = !empty($params['name']) ? $params['name'] : false;
    $date = !empty($params['date']) ? $params['date'] : false;
    $pawn_no = !empty($params['pawn_no']) ? $params['pawn_no'] : false;
    $apply_mount = !empty($params['apply_mount']) ? $params['apply_mount'] : false;
    $new_value = !empty($params['new_value']) ? $params['new_value'] : false;
    $pushMsgParams = array(//根据业务的发送场景自定义
        1 => "{\"name\":\"$name\",\"date\":\"$date\"}",//提款成功
        2 => "{\"name\":\"$name\",\"date\":\"$date\",\"pawn_no\":\"$pawn_no\"}",//抵押品出库成功
        3 => "{\"name\":\"$name\",\"date\":\"$date\",\"apply_mount\":\"$apply_mount\"}",//提款成功-信贷
        4 => "{\"name\":\"$name\",\"date\":\"$date\",\"new_value\":\"$new_value\"}",//出库成功-客户
        5 => "{\"name\":\"$name\",\"date\":\"$date\",\"new_value\":\"$new_value\"}",//出库成功-信贷
        6 => "{\"name\":\"$name\",\"date\":\"$date\"}",//出库成功-客户
        7 => "{\"name\":\"$name\",\"date\":\"$date\"}",//出库成功-信贷
        8 => "{\"name\":\"$name\",\"date\":\"$date\"}",//出库失败-信贷
        9 => "{\"name\":\"$name\"}",//授信成功-信贷
        10 => "{\"name\":\"$name\"}",//授信成功-风控
        11 => "{\"name\":\"$name\"}",//授信成功-信贷
        12 => "{\"name\":\"$name\"}",//授信成功-风控
    ...
    $pushMsgParam = $pushMsgParams[$type];
    //提取推送内容
    $scenes = C('PUSH_MSG_SCENE');
    $msg = $scenes[$type][1];
    $params_arr = json_decode($pushMsgParam);
