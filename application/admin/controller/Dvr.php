...
*/
        ...
        $this -> assign('doc',SITE.'/public/download/addMonitoring.doc');
        return $this -> fetch();
    } 
    ...
    public function add(){ 
        ...
        if(!IS_POST){
            ...
            $data = I('post.', array()); 
            extract($data);
            if(0 < M('store_dvr')->where(['ip'=>$ip,status=>1])->count()){
                $this->error('ip已存在!');exit;
            }
            //店铺信息
            ...
            $data['add_ip'] = get_user_ip();
 
            $result = M($model)->add($data);

            if(0 !== $result || false !== $result){
                //设置分组 
                M($model)->where(['id'=>$result])->setField('group', round(($result + 10)/10));
                //添加日志
                $log['action'] = 1;
                $log['admin_id'] = $admin_info['admin_id'];
                $log['admin_name'] = $admin_info['user_name'];
                $log['dvr_id'] = $result;
                $log['dvr_brand'] = $data['brand'];
                // $log['user_id'] = $user_id;
                ...
                M('store_dvr_log')->add($log);
                $this->success('添加成功', U('dvr/index'));
            }else{
                $this->error('添加失败');
            }
    ...
    public function edit(){
        ...
        if(!$info){
            $this->error('该录像机（DVR）信息不存在');
        }
        if(!IS_POST){ 
            $action_url = U('dvr/edit'); 
            //总管理员 返回 所有用户 风控经理返回自己的用户
            $where = array();
            if(4 == $admin_info['role_id']){ 
                $where['xd_id'] = $admin_id;
