...
function getAllAdmin(){
    $a_arr = array();
    $allList = M('admin') -> field('admin_id,user_name') -> select();
    if($allList){
        foreach ($allList as $key => $val){
            $a_arr[$val['admin_id']] = $val['user_name'];
        }
    }
    return $a_arr;
}
 ...
 */
//抵押品出库,当前抵押率异常的报警短信
function send_BJSMS($scene, $sender, $params, $unique_id=0)
{
    ...
    $usermobile = !empty($params['usermobile']) ? $params['usermobile'] : false;
    $now_rate = !empty($params['now_rate']) ? $params['now_rate'] : false;
    $check_rate = !empty($params['check_rate']) ? $params['check_rate'] : false;
    $pawn_no = !empty($params['pawn_no']) ? $params['pawn_no'] : false;
    $green_time = !empty($params['green_time']) ? $params['green_time'] : false;
    if(empty($unique_id)){
    ...
    $smsParams = array(//根据业务的发送场景自定义
        //当前抵押率异常
        1 => "{\"name\":\"$name\",\"username\":\"$username\",\"usermobile\":\"$usermobile\",\"now_rate\":\"$now_rate\",\"check_rate\":\"$check_rate\"}",
        //人工解压出库
        2 => "{\"name\":\"$name\",\"username\":\"$username\",\"usermobile\":\"$usermobile\",\"pawn_no\":\"$pawn_no\",\"green_time\":\"$green_time\",\"now_rate\":\"$now_rate\",\"check_rate\":\"$check_rate\"}"
    );

    $smsParam = $smsParams[$scene];
    //提取发送短信内容
    $scenes = C('BJ_SEND_SCENE');
    $msg = $scenes[$scene][1];
    ...
    //发送记录存储数据库
    $log_id = M('pawn_sms')->insertGetId(array('mobile' => $sender, 'code' => $code, 'add_time' => time(), 'session_id' => $session_id, 'status' => 0, 'scene' => $scene, 'msg' => $msg));
    if ($sender != '' && check_mobile($sender)) {//如果是正常的手机号码才发送
        ...
        if ($resp['status'] == 1) {
            M('pawn_sms')->where(array('id' => $log_id))->save(array('status' => 1)); //修改发送状态为成功
        }else{
            M('pawn_sms')->where(array('id' => $log_id))->update(array('error_msg'=>$resp['msg'])); //发送失败, 将发送失败信息保存数据库
        }
        return $resp;
    } else {
