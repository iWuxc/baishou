 ...
 */
function qiniuuploadv($tmp_name){
...
function filter_null($array){ 
    array_walk_recursive($array, function (& $val, $key ) { if ($val === null)  $val = ''; });
    return $array;
}
/** 
 * 使用PHP检测能否ping通IP或域名 
...
function pingAddress($address) {  
    $address = ltrim($address, 'http://');
    $address = ltrim($address, 'https://');
    $status = -1;  
...
function send_sms($mobile,$content){
    ...
    $url = "http://dx.106.pw/SendUserSMS.aspx?comid=10086&loginid=jxf15210900641&password=000000&mobile=$mobile&smscontent=$content&timerflag=0&timervalue=";  
    $result = file_get_contents($url);  
    // $result = httpRequest($url);  
    // print_r($result);die;
    if('0' == explode(':', $result)[1]){
        return true;
    }else{
        return false;
    } 
